...
def implicitNN_cosine(matrix, realmatrix):
    return implicitNN(matrix, realmatrix, implicit.nearest_neighbours.CosineRecommender())

def implicitNN_tfidf(matrix, realmatrix):
    return implicitNN(matrix, realmatrix, implicit.nearest_neighbours.TFIDFRecommender())

def implicitNN_bm25(matrix, realmatrix):
    return implicitNN(matrix, realmatrix, implicit.nearest_neighbours.BM25Recommender())

# Is working but really bad predictions....
def implicitNN(matrix, realmatrix, recommender):
