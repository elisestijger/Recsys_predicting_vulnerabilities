...
def novelty(predicted: List[list], pop: dict, u: int, n: int) -> (float, list):
    ...
    for sublist in predicted:
        ...
        k += 1
        # print(k)
        # sublist is the list with items for this user 
        for i in sublist:
            # log2 amount this items appears in dataset/amount of users in the training set -> "so probabilty this item will be recommended"
            self_information += np.sum(-np.log2(pop[i]/u))
      
        # average self info per user 
        mean_self_information.append(self_information/n)

    # average self info per fold (total all users this fold/amount of sublists (= users)  )
